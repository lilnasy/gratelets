...
export const visitors: SimpleVisitors<State> = {
    ImportDeclaration(node, state) {
        if (node.source.value !== "emotion:extract") return
        if (state.importedOnce) {
            throw new Error(
                "'emotion:extract' must only be imported once per file",
                { cause: `'${state.id}' imported 'emotion:extract' multiple times.` }
            )
        }
        ...
        for (const specifier of node.specifiers) {
            if (specifier.type === "ImportNamespaceSpecifier") {
                ...
                if (
                    ...
                    // import { css as localName } from "emotion:extract"
                    (imported.type ===  "Literal" && imported.value === "css")
                ) {
                    state.cssFunctionLocalName = local.name
                }
                else if (
                    // import { injectGlobal } from "emotion:extract"
                    (imported.type === "Identifier" && imported.name === "injectGlobal") ||
                    // import { injectGlobal as localName } from "emotion:extract"
                ...
                throw new StacklessError(
                    "'emotion:extract' does not export a default value. Did you mean to import 'css'?\n\n" +
                    "import { css } from 'emotion:extract'\n",
                    { cause: `'${state.id}' tried to import default as '${specifier.local.name}' from 'emotion:extract''` }
                )
            }
        }
